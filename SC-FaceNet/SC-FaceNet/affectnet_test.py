...
def parse_args():
    ...
    parser.add_argument('--aff_path', type=str, default='', help='AfectNet dataset path.')
...
def run_test():
    ...
    model = SCFaceNet(num_class=args.num_class, num_head=args.num_head, pretrained=False)
    ...
    if args.num_class == 7: 
        ...
        plt.savefig(os.path.join('checkpoints', f"affectnet7_acc{acc}.svg"),
                    format='svg',
                    bbox_inches='tight',
                    dpi=500)
        plt.close()
        ...
        plt.savefig(os.path.join('checkpoints', f"affectnet8_acc{acc}.svg"),
                    format='svg',
                    bbox_inches='tight',
                    dpi=500)
        plt.close()
