...
class SCFaceNet(nn.Module):
    def __init__(self, num_class=7, num_head=2, pretrained=True):
        super(SCFaceNet, self).__init__()

        net = backbone.Backbone()

        if pretrained:
            net = torch.load(os.path.join('./pretrained/', "SC-FaceNet.pth"), weights_only=False)

        self.features = nn.Sequential(*list(net.children())[:-4])
        self.num_head = num_head
        for i in range(int(num_head)):
            setattr(self, "cat_head%d" % (i), RFBAU())
    ...
    def forward(self, x):
        ...
        for i in range(self.num_head):
            BAU.append(getattr(self, "cat_head%d" % i)(x))
        head_out = BAU
